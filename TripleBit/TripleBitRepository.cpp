...
TripleBitRepository* TripleBitRepository::create(const string path)
{
	...
	if(OSFile::FileExists(filename.c_str()) == false) {
...
#endif
	resultSet.push_back("12");
	char buffer[20];
	sprintf(buffer, "%.3f", ((end_time.tv_sec - start_time.tv_sec) * 1000000.0 + (end_time.tv_usec - start_time.tv_usec)) / 1000.0);
	resultSet.push_back(buffer);
	resBegin = resultSet.begin(); resEnd = resultSet.end();
#endif
...
void TripleBitRepository::cmd_line(FILE* fin, FILE* fout, const string query_path)
{
	char cmd[256];
	while (true) {
		fflush(fin);
