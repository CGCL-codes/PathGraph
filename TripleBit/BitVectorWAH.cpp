...
void BitVectorWAH::encode(unsigned short int units)
{
	...
	for (i = 0; i < units; i++) {
		int temp = i;
		bool value;
		if (parseBit(temp, value) == false) {//不可压缩直接存储
			temp = temp | 0x80;
			insertValue(temp);
		} else {
...
void BitVectorWAH::increaseOnesCount(word& unit)
{
	...
	count = count + 1;
	if(count > 0x3F){ //原来的count计数位为3F,需要将计数插入到下一个字节
		temp = 0;
		temp = temp | 0x40;
		temp = temp | 1;
...
/*
 * 返回pos所在的字节在chunk中的位置
 * pos: 输入参数，要得到的值的位置, 返回在value中的位置
 * value: pos所在字节的值
 * isCompressed 标志该字节是否被压缩
 * ChunkNO: 字节所在的ChunkNo
 */
