...
public:
	...
	// set the repository path and query path;
	...
	typedef vector<string>::const_iterator result_iterator;
	result_iterator get_result_begin() const { return resultSet.begin(); }
	result_iterator get_result_end() const { return resultSet.end(); }

	void cmd_line(FILE* fin, FILE* fout, const string query_path);
	static int colNo;
};
