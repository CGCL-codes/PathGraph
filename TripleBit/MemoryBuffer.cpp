...
MemoryBuffer::MemoryBuffer(unsigned _size) :size(_size) {
	...
	if(buffer == NULL) {
		MessageEngine::showMessage("MemoryBuffer::MemoryBuffer, malloc error!", MessageEngine::ERR);
	}
	::memset(buffer, 0, size);
	currentHead = buffer;
}

MemoryBuffer::MemoryBuffer(unsigned size, char* buf){
	buffer = buf;
	this->size = size;
}
...
MemoryBuffer::~MemoryBuffer() {
	...
	//free the buffer
//	printf("MemoryBuffer address:%x size:%u\n",buffer,size);
	if(buffer != NULL)
		free(buffer);
	buffer = NULL;
	size = 0;
...
char* MemoryBuffer::resize(unsigned increaseSize)
{
	...
	if(buffer == NULL) {
		MessageEngine::showMessage("MemoryBuffer::addBuffer, realloc error!", MessageEngine::ERR);
	}
...
void MemoryBuffer::load(ifstream& ifile)
{
	...
	if(buffer == NULL) {
		MessageEngine::showMessage("MemoryBuffer::load, malloc error!", MessageEngine::ERR);
	}
	...
	while(remainSize > 0) {
		...
		if(buffer == NULL) {
			MessageEngine::showMessage("MemoryBuffer::load, realloc error!", MessageEngine::ERR);
		}
...
{
	if(REIFICATION_INIT_PAGE_COUNT * MemoryBuffer::pagesize / sizeof(ID) <= pos){
