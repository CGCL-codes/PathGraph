...
//	virtual Status getAllPredicatesByID(unsigned id, ID* & buffer,int&) = 0;
...
public:
	...
	Status getAllPredicatesByID(unsigned id, ID buffer[18],int&);
	...
	static TwoConstantStatisticsBuffer* loadone(StatisticsType type, const string path, char*& indexBuffer);
private:
	/// get the value length in bytes;
	unsigned getLen(unsigned v);
	/// decode a chunk
