...
EntityIDBuffer::EntityIDBuffer() {
	...
	buffer = (ID*) malloc(ENTITY_BUFFER_INIT_PAGE_COUNT
			* MemoryBuffer::pagesize);
	p = buffer;
	usedSize = 0;
	sizePerPage = MemoryBuffer::pagesize / sizeof(ID);
...
EntityIDBuffer* EntityIDBuffer::copy( EntityIDBuffer *otherBuffer) {
	if (this != otherBuffer) {
		usedSize = otherBuffer->usedSize;
...
Status EntityIDBuffer::insertID(ID id) {
	if (usedSize >= totalSize) {
		buffer = (ID*) realloc((char*) buffer, totalSize * sizeof(ID)
				+ ENTITY_BUFFER_INCREMENT_PAGE_COUNT * MemoryBuffer::pagesize);
		//memset((char*)(buffer + totalSize), 255, ENTITY_BUFFER_INCREMENT_PAGE_COUNT * getpagesize());

		p = buffer + totalSize;

		totalSize = totalSize + ENTITY_BUFFER_INCREMENT_PAGE_COUNT
				* sizePerPage;
		pos = 0;
	}/*else if(totalSize > 10485760 && 2*usedSize < totalSize) {
		totalSize =  usedSize  + ENTITY_BUFFER_INCREMENT_PAGE_COUNT * sizePerPage;
		buffer = (ID*) realloc((char*) buffer, totalSize* sizeof(ID));
	}*/
...
Status EntityIDBuffer::sort() {
	...
	for (i = 0; i < chunkCount; i++) {
		...
		if (i == chunkCount - 1)
			CThreadPool::getInstance().AddTask(boost::bind(&SortTask::Run, p,
					getSize() - chunkSize * (chunkCount - 1), sortKey, IDCount));
		else
			CThreadPool::getInstance().AddTask(boost::bind(&SortTask::Run, p,
					chunkSize, sortKey, IDCount));
	}
	...
	while (1) {
		...
		for (i = 0; i < chunkCount; i += slot) {
			if (i + slot == chunkCount) {
				CThreadPool::getInstance().AddTask(boost::bind(
						&EntityIDBuffer::merge, this, i * chunkSize, (i + slot
								/ 2 - 1) * chunkSize, (i + slot / 2)
								* chunkSize, getSize(), tempBuffer));
			} else {
				CThreadPool::getInstance().AddTask(
						boost::bind(&EntityIDBuffer::merge, this,
								i * chunkSize, (i + slot / 2 - 1) * chunkSize,
								(i + slot / 2) * chunkSize, (i + slot)
										* chunkSize, tempBuffer));
			}
		}
...
void EntityIDBuffer::merge(int start1, int end1, int start2, int end2,
	...
	if (IDCount == 1) {
		...
		if (sortKey == 0) {
			mergeBuffer(result, startPtr1, startPtr2, (start2 - start1), (end2
					- start2), 2, 0);
		} else if (sortKey == 1) {
			mergeBuffer(result, startPtr1, startPtr2, (start2 - start1), (end2
					- start2), 2, 1);
...
Status EntityIDBuffer::mergeSingleBuffer(EntityIDBuffer* entbuffer ,EntityIDBuffer* entbuffer1,EntityIDBuffer* entbuffer2){
	return 	mergeSingleBuffer(entbuffer,entbuffer1->buffer,entbuffer2->buffer,entbuffer1->getSize(),entbuffer2->getSize());
}
...
void EntityIDBuffer::quickSort(ID* p, int size) {
	CThreadPool::getInstance().AddTask(boost::bind(&SortTask::Run, p, size,
			sortKey, IDCount));
}
...
size_t EntityIDBuffer::getEntityIDPos(ID id) {
	...
	while (low <= high) {
		...
		if (buffer[mid * IDCount + sortKey] == id) {
			while (mid != size_t(-1) && buffer[mid * IDCount + sortKey] == id)
				mid--;
			mid++;
			return (size_t) mid;
...
ID EntityIDBuffer::findValueByKey(ID key){
	longlong low = 0, high = getSize() - 1;
	longlong mid;

	while (low <= high) {
		mid = low + (high - low) / 2;
		if (buffer[mid * IDCount + sortKey] == key) {
			while (mid != size_t(-1) && buffer[mid * IDCount + sortKey] == key){
				mid--;
			}
			mid++;
			return buffer[mid * IDCount + 1 - sortKey];
		} else if (buffer[mid * IDCount + sortKey] > key)
			high = mid - 1;
		else
			low = mid + 1;
	}
...
void EntityIDBuffer::findValuesByKey(ID key, vector<ID> &startID){
	startID.clear();
	longlong low = 0, high = getSize() - 1;
	longlong mid;
	...
	while (low <= high) {
		...
		if (buffer[mid * IDCount + sortKey] == key) {
			longlong t = mid + 1;
			while (buffer[mid * IDCount + sortKey] == key){
				startID.push_back(buffer[mid * IDCount + 1 - sortKey]);
				if(!mid)break;
				mid--;
			}

			while(t < getSize() && buffer[t * IDCount + sortKey] == key){
				startID.push_back(buffer[t * IDCount + 1 - sortKey]);
				t++;
			}
			break;
		} else if (buffer[mid * IDCount + sortKey] > key)
...
bool EntityIDBuffer::modifyValueByKey(ID key, ID value){
	longlong low = 0, high = getSize() - 1;
	longlong mid;
	...
	while (low <= high) {
		...
		if (buffer[mid * IDCount + sortKey] == key) {
			while (mid != size_t(-1) && buffer[mid * IDCount + sortKey] == key)
				mid--;
			mid++;
			buffer[mid * IDCount + 1 - sortKey] = value;
			return true;
		} else if (buffer[mid * IDCount + sortKey] > key)
			high = mid - 1;
		else
...
//TODO merge join;
Status EntityIDBuffer::mergeIntersection(EntityIDBuffer* entBuffer,
		char* flags, ID joinKey) {
	joinKey--;
...
Status EntityIDBuffer::intersection(EntityIDBuffer* entBuffer, char* flags,
	...
	ID* temp2 = (ID*) malloc(MemoryBuffer::pagesize);
	...
	size_t totalPerPage = MemoryBuffer::pagesize / 4;
...
int SortTask::Run(ID* p, size_t length, int sortKey, int IDCount) {
	if (IDCount == 1) {
		...
		if (sortKey == 0) {
			qsort((I64*) &p[0], length, sizeof(I64), qcompareLongByFirst32);
		} else if (sortKey == 1) {
			qsort((I64*) &p[0], length, sizeof(I64), qcompareLongBySecond32);
		}
	}
...
bool SortTask::compareLongByFirst32(I64 a, I64 b) {
	ID* ptr1 = (ID*) &a;
	ID* ptr2 = (ID*) &b;
...
bool SortTask::compareLongBySecond32(I64 a, I64 b) {
	ID* ptr1 = (ID*) &a;
	ID* ptr2 = (ID*) &b;
...
Status EntityIDBuffer::modifyByFlag(char* flag, int No) {
	...
	ID* temp = (ID*) malloc(MemoryBuffer::pagesize);
	ID* p = buffer;
	...
	for (j = 0; j < size; j++) {
		if (flag[j] == No) {
			if (pos == totalPerPage) {
				memcpy(buffer + writeSize, temp, MemoryBuffer::pagesize);
				writeSize += pos;
				pos = 0;
			}
			for (n = 0; n < IDCount; n++) {
...
Status EntityIDBuffer::mergeBuffer(EntityIDBuffer* XTemp,
	...
	if (buffer == NULL) {
		buffer = (ID*) malloc(MemoryBuffer::pagesize);
		for (int i = 1; i < pageCount; i++) {
			buffer = (ID*) realloc(buffer, (i + 1) * MemoryBuffer::pagesize);
			if (buffer == NULL)
